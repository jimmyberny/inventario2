 ...
 * @author Enrique
 */
public class BuscadorPrograma extends Buscador<Programa> {
	
	private static final Logger log = LoggerFactory.getLogger(BuscadorPrograma.class);
	
	private DatosGeneral dGeneral;
	
	public BuscadorPrograma(Frame parent, boolean modal) {
		super(parent, modal);
	}
	
	public BuscadorPrograma(Dialog parent, boolean modal) {
		super(parent, modal);
	}
	
	public static BuscadorPrograma mostrar(Component padre, Aplicacion app) {
		BuscadorPrograma bt;
		Window mw = SwingUtilities.getWindowAncestor(padre);
		if (mw instanceof Frame) {
			bt = new BuscadorPrograma((Frame) mw, true);
		} else {
			bt = new BuscadorPrograma((Dialog) mw, true);
		}
		bt.initComponents();
		bt.init(app);
    ...
    private void initComponents() {
        ...
        jspItems.setViewportView(jlProgramas);
    ...
    private void jbAceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbAceptarActionPerformed
		try {
			aceptar(jlProgramas.getSelectedIndex());
		} catch (InventarioException ex) {
