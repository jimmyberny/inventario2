...
public class Principal extends javax.swing.JFrame implements Aplicacion, ActionListener {
    ...
    private void generarAutoeventos() {
        try {
            ...
            // Intentar enviar los e-mails corresponientes
            for (Evento evento : eventos) {
                Mensaje m = new Mensaje();
                m.setDestinatario(conf.getProperty(InventarioApp.KC_EMAIL_USUARIO));
                m.setAsunto(evento.getNombre());
                m.setMensaje(evento.getInstruccion());
    ...
    @Override
    public void enviarMensaje(Mensaje mensaje) throws InventarioException {
        //
        final String smtpUsuario = conf.getProperty(InventarioApp.KC_EMAIL_USUARIO);
        final String smtpClave = conf.getProperty(InventarioApp.KC_EMAIL_CLAVE);
        log.info("{} with {}", smtpUsuario, smtpClave);

        Properties props = new Properties();
        props.put("mail.smtp.auth", "true");
        props.put("mail.smtp.starttls.enable", "true");
        props.put("mail.smtp.host", conf.getProperty(InventarioApp.KC_SMTP_HOST));
        props.put("mail.smtp.port", conf.getProperty(InventarioApp.KC_SMTP_PORT));
        ...
        Authenticator auth = new Authenticator() {
            ...
            protected javax.mail.PasswordAuthentication getPasswordAuthentication() {
                return new javax.mail.PasswordAuthentication(smtpUsuario, smtpClave);
            }
        ...
        try {
            ...
            String dest = mensaje.getDestinatario();
            if (dest == null) {
                // Si el mensaje no tiene destinatario usar el del usuario de la aplicacion
                dest = smtpUsuario;
            }

            Message message = new MimeMessage(session);
            message.setFrom(new InternetAddress(smtpUsuario));
            ...
            throw new InventarioException("No se ha podido enviar el email", mex);
        }
    }
